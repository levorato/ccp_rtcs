...
def create_trace_scenario_filename(model, Gamma_perc, test_name, instance_name, sim_strategy, model_policy, reoptimize, scenario_id):
    if model == "robust-budget" or model == "deterministic":
...
def read_concatenated_trace_df(result_path, model_name, result_type = ""):
    ...
    for file in glob.glob(os.path.join(result_path.strip(), "*.zip")):
        if (pathlib.Path(file).suffix == ".zip") and (model_name in file):
            ...
            output_file_trace_arrow = pathlib.Path(trace_file_zip).stem
            if result_type == "var":
                output_file_trace_arrow += "_var.arrow"
            else:
                output_file_trace_arrow += ".arrow"
...
if run_from_collab:
    ...
    gdrive_folder = '/projetos/levorato'
...
experiment_list = ["run_sim_japan_forecast_avg"]  # ["run_sim"]  # "run_sim_japan_forecast_avg"
instance_group_list = ["japan-10"]  # ["antoine-skew"]  # "japan-10"
...
for result_type in result_type_list:
    for instance_group, experiment_folder, experiment in zip(instance_group_list, experiment_folder_list, experiment_list):
        ...
        robust_model_names = ['robust-budget_{}'.format(_) for _ in [0, 20, 40, 60, 80, 100]]  # [0, 20, 30, 40, 50, 60, 80, 100]]
        for model_name in robust_model_names + det_model_names + ['robust-box']:
            ...
            for instance_path in instances_to_process:
                ...
                df = read_concatenated_trace_df(result_path, model_name, result_type)
                ...
                if len(df.index) > 0:
                    outfilepath = os.path.join(reportfolder_df, experiment + '.' + instance_group + '.' + instance_name + '.' + model_name)
                    if result_type == "var":
                        outfilepath += '.var-results.pkl.gz'
                    else:
                        outfilepath += '.cost-results.pkl.gz'
