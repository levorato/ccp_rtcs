# ==================================================================
# RCCP_Robust_Budget.jl
# RCCP Robust MILP Model Based on Budgeted Uncertainty Set
# ==================================================================
using JuMP
using CSV
using DataFrames
...
function solve_robust_model_budget(instance_as_dict::Dict, general_logger, infeasible_logger, solver_params, Gamma, t0 = 1,
                                fixed_contracts = Int64[], initial_battery = Float64[], previous_drivable_charge = Float64[])
    ...
    println("\n===========  R O B U S T    B U D G E T    M O D E L  ( t0 = $(t0) ) ===========\n")
    ...
    println("Budget of uncertainty Gamma is $(Gamma)")
    ...
    if size(initial_battery, 1) > 0
        for s in ST
            ...
            if (initial_battery[s] - storage[s,:uMin] < -EPS)
                ...
                println(general_logger, "[Robust Budget Model, t0 = $(t0)] WARN: assert initial_battery[s] >= storage[s,:uMin] : $(initial_battery[s]) < $(storage[s,:uMin])")
                if abs(initial_battery[s] - storage[s,:uMin]) / storage[s,:uMin] <= RTCS_ROUNDING_ERROR
                    initial_battery[s] = storage[s,:uMin]
                else
                    println(general_logger, "[Robust Budget Model, t0 = $(t0)] initial_battery[s] must be >= storage[s,:uMin]. Difference exceeds rounding error of $(RTCS_ROUNDING_ERROR).")
            ...
            if (initial_battery[s] - storage[s,:uMax] > EPS)
                ...
                println(general_logger, "[Robust Budget Model, t0 = $(t0)] WARN: assert initial_battery[s] <= storage[s,:uMax] : $(initial_battery[s]) > $(storage[s,:uMax])")
                if abs(initial_battery[s] - storage[s,:uMax]) / storage[s,:uMax] <= RTCS_ROUNDING_ERROR
                    initial_battery[s] = storage[s,:uMax]
                else
                    println(general_logger, "[Robust Budget Model, t0 = $(t0)] initial_battery[s] must be <= storage[s,:uMax] Difference exceeds rounding error of $(RTCS_ROUNDING_ERROR).")
                    flush(general_logger)
        ...
        println("[Robust Budget Model, t0 = $(t0)] Fixing battery level variables to $(initial_battery)")
    end
    # Initial drivable charge fixation
    if size(previous_drivable_charge, 1) > 0
        println("[Robust Budget Model, t0 = $(t0)] Fixing drivable initial charge to $(previous_drivable_charge)")
    end
    ...
    # ============== Budgeted Uncertainty Gamma =============
    # TODO FIXME When reoptimizing (starting at t0), do we have to subtract
    # from Gamma_budget the number of deviations occured so far ?
    # ===============================================================

    # TODO: Acrescentar o seguinte calculo ao resultado final da FO (termo constante): sum(s in B, v[s] * (r0[s] - r[t_bar,s]))

    # FIXME Assuming r0[s] == r0[1,s]
    # ============== Objective Function Constraint ===================
    # (34): E >= sum(t in T, c in C_t, v[t,c] y[t,c] + alpha[t,c] q[t,c](.)) + sum(s in D, v[s] * sum(t in T, P[t,s] x[t,s](.))) + sum(s in UND, v[s] * sum(t in T, P_hat[t,s]))
    #             + sum(s in B, v[s] * (r0[s] - r[t_bar,s] + sum(t in T, (1 + lambda[s]) * g[t,s](.)))) + sum(t in T, beta[t] e[t](.))
    @variable(mrob, E)
    @variable(mrob, f0_34)
    ...
    #println(x)
    # ============== Dual budget constraints for eqn (34) =========== Max Case (<=)
    @variable(mrob, u_34[tau=SetTau, sigma=SetSigma] >= 0)
    @variable(mrob, v_34 >= 0)
    @constraint(mrob, f0_34 + v_34 * Gamma + sum(u_34[tau,sigma] + v_bar[tau, sigma] * f_34[tau, sigma]
                for tau in SetTau, sigma in SetSigma) <= 0)
    @constraint(mrob, [tau=SetTau, sigma=SetSigma], u_34[tau, sigma] + v_34 >= f_34[tau, sigma]*v_hat[tau, sigma])
    @constraint(mrob, [tau=SetTau, sigma=SetSigma], u_34[tau, sigma] + v_34 >= -f_34[tau, sigma]*v_hat[tau, sigma])
    #@constraint(mrob, [tau=SetTau, sigma=SetSigma], n_34[tau, sigma] >= 0)
    # ===============================================================

    # (35): sum(c in C_t, q[t,c](.)) + sum(s in ND, P[t,s]) + sum(s in UND, P_hat[t,s]) + sum(s in D, P[t, s] x[t,s](.)) + sum(s in B, (lambda[s] - 1) g[t,s](.)) + r[t,s](. ; t-1) - r[t+1,s](.) + e[t](.) >= 0, forall t in T.
    ...
    @constraint(mrob, [t=T, sigma=SetSigma], f_35[t,t,sigma] ==
        ...
        + e_[t,t,sigma])
    # ============== Dual budget constraints for eqn (35) =========== Min Case (>=)
    # Case when robust inequality is >= 0
    @variable(mrob, u_35[t=T, tau=t0:t, SetSigma] <= 0)
    @variable(mrob, v_35[t=T] <= 0)
    @constraint(mrob, [t=T], f0_35[t] + sum(v_bar[tau, sigma] * f_35[t, tau, sigma] + u_35[t, tau, sigma] for tau in t0:t, sigma in SetSigma)
                + v_35[t] * Gamma >= 0)
    @constraint(mrob, [t=T, tau=t0:t, sigma=SetSigma], u_35[t, tau, sigma] + v_35[t] <= f_35[t, tau, sigma]*v_hat[tau, sigma])
    @constraint(mrob, [t=T, tau=t0:t, sigma=SetSigma], u_35[t, tau, sigma] + v_35[t] <= -f_35[t, tau, sigma]*v_hat[tau, sigma])
    #@constraint(mrob, [t=T, tau=SetTau, sigma=SetSigma], n_35[t, tau, sigma] >= 0)

    # (36): lambda[s] g[t,s](.) - r[t+1,s](.) <= 0, forall t in T, s in B
    ...
    # ============== Dual budget constraints for eqn (36) =========== Max Case (<=)
    @variable(mrob, u_36[t=T, ST, tau=t0:t, SetSigma] >= 0)
    @variable(mrob, v_36[t=T, ST] >= 0)
    @constraint(mrob, [t=T, s=ST], 0 >= f0_36[t, s] + sum(v_bar[tau, sigma] * f_36[t, s, tau, sigma] + u_36[t, s, tau, sigma]
                for tau in t0:t, sigma in SetSigma) + v_36[t, s] * Gamma )
    @constraint(mrob, [t=T, s=ST, tau=t0:t, sigma=SetSigma], u_36[t, s, tau, sigma] + v_36[t, s] >= -f_36[t, s, tau, sigma]*v_hat[tau, sigma])
    @constraint(mrob, [t=T, s=ST, tau=t0:t, sigma=SetSigma], u_36[t, s, tau, sigma] + v_36[t, s] >= f_36[t, s, tau, sigma]*v_hat[tau, sigma])

    # TODO o que fazer aqui ?
    #@variable(mrob, f0_37)
    # (37): r_0[s] == u_0(s), forall s in B
    ...
    # (38a): r[t,s](.) >= u_min[s], forall t in T + {t_bar}, s in B
    ...
    # ============== Dual budget constraints for eqn (38a) =========== Min Case (>=)
    @variable(mrob, u_38a[t=t0:nbT+1, ST, tau=t0:t-1, SetSigma] <= 0)
    @variable(mrob, v_38a[t=t0:nbT+1, ST] <= 0)
    @constraint(mrob, [t=t0:nbT+1, s=ST], f0_38a[t, s] + sum((v_bar[tau, sigma] * f_38a[t, s, tau, sigma]
                + u_38a[t, s, tau, sigma]) for tau in t0:t-1, sigma in SetSigma) + v_38a[t, s] * Gamma <= 0 )
    @constraint(mrob, [t=t0:nbT+1, s=ST, tau=t0:t-1, sigma=SetSigma], u_38a[t, s, tau, sigma] + v_38a[t, s] <= -f_38a[t, s, tau, sigma]*v_hat[tau, sigma])
    @constraint(mrob, [t=t0:nbT+1, s=ST, tau=t0:t-1, sigma=SetSigma], u_38a[t, s, tau, sigma] + v_38a[t, s] <= f_38a[t, s, tau, sigma]*v_hat[tau, sigma])

    # TODO: Rever a questao da somatoria de Tau ser de 0 a t (como resolver o indice 0 em Julia)
    # TODO Resolver o T = T_{bar}!
    # (38b): r[t,s](.) <= u_max[s], forall t in T + {t_bar}, s in B
    ...
    # TODO verificar o tau in 1:(nbTau-1)
    # ============== Dual budget constraints for eqn (38b) =========== Max Case (<=)
    @variable(mrob, u_38b[t=t0:nbT+1, ST, tau=t0:t-1, SetSigma] >= 0)
    @variable(mrob, v_38b[t=t0:nbT+1, ST] >= 0)
    @constraint(mrob, [t=t0:nbT+1, s=ST], f0_38b[t, s] + sum((v_bar[tau, sigma] * f_38b[t, s, tau, sigma]
                + u_38b[t, s, tau, sigma]) for tau in t0:t-1, sigma in SetSigma) + v_38b[t, s] * Gamma <= 0 )
    @constraint(mrob, [t=t0:nbT+1, s=ST, tau=t0:t-1, sigma=SetSigma], u_38b[t, s, tau, sigma] + v_38b[t, s] >= -f_38b[t, s, tau, sigma]*v_hat[tau, sigma])
    @constraint(mrob, [t=t0:nbT+1, s=ST, tau=t0:t-1, sigma=SetSigma], u_38b[t, s, tau, sigma] + v_38b[t, s] >= f_38b[t, s, tau, sigma]*v_hat[tau, sigma])

    # TODO Corrigir a terceira linha abaixo no artigo em Latex
    # (39): lambda[s] g[t,s](.) + r[t,s](. ; t-1) - r[t+1,s](.) >= 0, forall t in T, s in B
    ...
    @constraint(mrob, [t=T, s=ST, sigma=SetSigma], f_39[t, s, t, sigma] == lambda[s]*g_[t, s, t, sigma]
        - r_[t+1,s,t,sigma])
    # ============== Dual budget constraints for eqn (39) =========== Min Case (>=)
    @variable(mrob, u_39[t=T, ST, tau=t0:t, SetSigma] <= 0)
    @variable(mrob, v_39[t=T, ST] <= 0)
    @constraint(mrob, [t=T, s=ST], f0_39[t, s] + sum((v_bar[tau, sigma] * f_39[t, s, tau, sigma]
                + u_39[t, s, tau, sigma]) for tau in t0:t, sigma in SetSigma) + v_39[t, s] * Gamma >= 0 )
    @constraint(mrob, [t=T, s=ST, tau=t0:t, sigma=SetSigma], u_39[t, s, tau, sigma] + v_39[t, s] <= -f_39[t, s, tau, sigma]*v_hat[tau, sigma])
    @constraint(mrob, [t=T, s=ST, tau=t0:t, sigma=SetSigma], u_39[t, s, tau, sigma] + v_39[t, s] <= f_39[t, s, tau, sigma]*v_hat[tau, sigma])

    # (40): g[t,s](.) <= theta_abs[s] delta[t], forall t in T, s in B
    ...
    # ============== Dual budget constraints for eqn (40) =========== Max Case (<=)
    @variable(mrob, u_40[t=T, ST, tau=t0:t, SetSigma] >= 0)
    @variable(mrob, v_40[t=T, ST] >= 0)
    @constraint(mrob, [t=T, s=ST], f0_40[t, s] + sum((v_bar[tau, sigma] * f_40[t, s, tau, sigma]
                + u_40[t, s, tau, sigma]) for tau in t0:t, sigma in SetSigma) + v_40[t, s] * Gamma <= 0 )
    @constraint(mrob, [t=T, s=ST, tau=t0:t, sigma=SetSigma], u_40[t, s, tau, sigma] + v_40[t, s] >= -f_40[t, s, tau, sigma]*v_hat[tau, sigma])
    @constraint(mrob, [t=T, s=ST, tau=t0:t, sigma=SetSigma], u_40[t, s, tau, sigma] + v_40[t, s] >= f_40[t, s, tau, sigma]*v_hat[tau, sigma])

    # (41): lambda[s] g[t,s](.) + r[t,s](. ; t-1) - r[t+1, s](.) <= theta_ref[s] delta[t], forall t in T, s in B
    ...
    # ============== Dual budget constraints for eqn (41) =========== Max Case (<=)
    @variable(mrob, u_41[t=T, ST, tau=t0:t, SetSigma] >= 0)
    @variable(mrob, v_41[t=T, ST] >= 0)
    @constraint(mrob, [t=T, s=ST], f0_41[t, s] + sum((v_bar[tau, sigma] * f_41[t, s, tau, sigma]
                + u_41[t, s, tau, sigma]) for tau in t0:t, sigma in SetSigma) + v_41[t, s] * Gamma <= 0)
    @constraint(mrob, [t=T, s=ST, tau=t0:t, sigma=SetSigma], u_41[t, s, tau, sigma] + v_41[t, s] >= -f_41[t, s, tau, sigma]*v_hat[tau, sigma])
    @constraint(mrob, [t=T, s=ST, tau=t0:t, sigma=SetSigma], u_41[t, s, tau, sigma] + v_41[t, s] >= f_41[t, s, tau, sigma]*v_hat[tau, sigma])
    ...
    # (42): Pi_minus[c] y[t,c] <= q[t,c](.) <= Pi_plus[c] y[t,c], forall t in T, c in C_t, Pi_plus[c] > 0
    # (43): Pi_plus[c] y[t,c] <= q[t,c](.) <= Pi_minus[c] y[t,c], forall t in T, c in C_t, Pi_plus[c] < 0
    # First, we model the lower bounds of contract usage: e.g., q[t,c](.) - Pi_minus[c] y[t,c] >= 0
    @variable(mrob, f0_42a[t=T, c=1:num_contracts[t]])
    @variable(mrob, f_42a[t=T, c=1:num_contracts[t], tau=t0:t, SetSigma])
    @variable(mrob, u_42a[t=T, c=1:num_contracts[t], tau=t0:t, SetSigma] <= 0)
    @variable(mrob, v_42a[t=T, c=1:num_contracts[t]] <= 0)
    for t in T, c in 1:num_contracts[t]
        ...
        # Case when robust inequality is >= 0
        # ============== Dual budget constraints for eqn (42a) =========== Min Case (>=)
        @constraint(mrob, f0_42a[t, c] + sum((v_bar[tau, sigma] * f_42a[t, c, tau, sigma]
                    + u_42a[t, c, tau, sigma]) for tau in t0:t, sigma in SetSigma) + v_42a[t, c] * Gamma >= 0)
        @constraint(mrob, [tau=t0:t, sigma=SetSigma], u_42a[t, c, tau, sigma] + v_42a[t, c] <= -f_42a[t, c, tau, sigma]*v_hat[tau, sigma])
        @constraint(mrob, [tau=t0:t, sigma=SetSigma], u_42a[t, c, tau, sigma] + v_42a[t, c] <= f_42a[t, c, tau, sigma]*v_hat[tau, sigma])
    end

    # Second, we model the upper bounds of contract usage: e.g., q[t,c](.) - Pi_plus[c] y[t,c] <= 0
    @variable(mrob, f0_42b[t=T, c=1:num_contracts[t]])
    @variable(mrob, f_42b[t=T, c=1:num_contracts[t], tau=t0:t, SetSigma])
    @variable(mrob, u_42b[t=T, c=1:num_contracts[t], tau=t0:t, SetSigma] >= 0)
    @variable(mrob, v_42b[t=T, c=1:num_contracts[t]] >= 0)
    for t in T, c in 1:num_contracts[t]
        if pi_plus[t, c] > 0
        ...
        # Case when inequality is <= 0
        # ============== Dual budget constraints for eqn (42b) =========== Max Case (<=)
        @constraint(mrob, f0_42b[t, c] + sum(v_bar[tau, sigma] * f_42b[t, c, tau, sigma]
                    + u_42b[t, c, tau, sigma] for tau in t0:t, sigma in SetSigma) + v_42b[t, c] * Gamma <= 0)
        @constraint(mrob, [tau=t0:t, sigma=SetSigma], u_42b[t, c, tau, sigma] + v_42b[t, c] >= -f_42b[t, c, tau, sigma]*v_hat[tau, sigma])
        @constraint(mrob, [tau=t0:t, sigma=SetSigma], u_42b[t, c, tau, sigma] + v_42b[t, c] >= f_42b[t, c, tau, sigma]*v_hat[tau, sigma])
    end
    ...
    # (44): sum(t' <= t, P[t',s] x[t', s](.) >= Pmin[t,s]), forall t in T, s in D, P[t,s] > 0
    # (45): sum(t' <= t, P[t',s] x[t', s](.) <= Pmin[t,s]), forall t in T, s in D, P[t,s] < 0
    @variable(mrob, u_44[t=T, D, tau=t0:t, SetSigma])
    @variable(mrob, v_44[t=T, D])
    for t in T, s in D
        previous_charge = 0.0
        if size(previous_drivable_charge, 1) > 0
        ...
        if P_D[t,s] > 0
            if Pmin[t,s] > Pmax[t,s]
                println(general_logger, "[Robust Budget Model, t0 = $(t0)] Assertion error : [drivable D] P[t,s] > 0 => Pmin <= Pmax")
            end
            ...
            # ============== Dual budget constraints for eqn (44) =========== Min Case (>=)
            @constraint(mrob, [tau=t0:t, sigma=SetSigma], u_44[t, s, tau, sigma] <= 0)
            @constraint(mrob, v_44[t, s] <= 0)
            @constraint(mrob, f0_44[t, s] + sum((v_bar[tau, sigma] * f_44[t, s, tau, sigma]
                        + u_44[t, s, tau, sigma]) for tau in t0:t, sigma in SetSigma) + v_44[t, s] * Gamma >= 0)
            @constraint(mrob, [tau=t0:t, sigma=SetSigma], u_44[t, s, tau, sigma] + v_44[t, s] <= -f_44[t, s, tau, sigma]*v_hat[tau, sigma])
            @constraint(mrob, [tau=t0:t, sigma=SetSigma], u_44[t, s, tau, sigma] + v_44[t, s] <= f_44[t, s, tau, sigma]*v_hat[tau, sigma])
        else
            if Pmin[t,s] < Pmax[t,s]
                println(general_logger, "[Robust Budget Model, t0 = $(t0)] Assertion error : [drivable D] P[t,s] < 0 => Pmin >= Pmax")
            ...
            @constraint(mrob, [tau=t0:t, sigma=SetSigma], f_44[t, s, tau,sigma] ==
                sum(P_D[t1,s] * x_[t1,s,tau,sigma] for t1 in tau:t))
            # ============== Dual budget constraints for eqn (44) =========== Max Case (<=)
            @constraint(mrob, [tau=t0:t, sigma=SetSigma], u_44[t, s, tau, sigma] >= 0)
            @constraint(mrob, v_44[t, s] >= 0)
            @constraint(mrob, f0_44[t, s] + sum((v_bar[tau, sigma] * f_44[t, s, tau, sigma]
                        + u_44[t, s, tau, sigma]) for tau in t0:t, sigma in SetSigma) + v_44[t, s] * Gamma <= 0)
    ...
    # (47a): r[t,s](. ; t-1) >= 0, forall t in T, s in B
    ...
    # ============== Dual budget constraints for eqn (47a) =========== Min Case (>=)
    @variable(mrob, u_47a[t=(t0+1):(nbT+1), s=ST, tau=t0:t-1, SetSigma] <= 0)
    @variable(mrob, v_47a[t=(t0+1):(nbT+1), s=ST] <= 0)
    @constraint(mrob, [t=(t0+1):(nbT+1), s=ST], f0_47a[t, s] + sum((v_bar[tau, sigma] * f_47a[t, s, tau, sigma]
                + u_47a[t, s, tau, sigma]) for tau in t0:t-1, sigma in SetSigma) + v_47a[t, s] * Gamma >= 0)
    @constraint(mrob, [t=(t0+1):(nbT+1), s=ST, tau=t0:t-1, sigma=SetSigma], u_47a[t, s, tau, sigma] + v_47a[t, s] <= -f_47a[t, s, tau, sigma]*v_hat[tau, sigma])
    @constraint(mrob, [t=(t0+1):(nbT+1), s=ST, tau=t0:t-1, sigma=SetSigma], u_47a[t, s, tau, sigma] + v_47a[t, s] <= f_47a[t, s, tau, sigma]*v_hat[tau, sigma])

    # (47b): g[t,s](.) >= 0, forall t in T, s in B
    ...
    # ============== Dual budget constraints for eqn (47b) =========== Min Case (>=)
    @variable(mrob, u_47b[t=T, s=ST, tau=t0:t, SetSigma] <= 0)
    @variable(mrob, v_47b[t=T, s=ST] <= 0)
    @constraint(mrob, [t=T, s=ST], f0_47b[t, s] + sum((v_bar[tau, sigma] * f_47b[t, s, tau, sigma]
                + u_47b[t, s, tau, sigma]) for tau in t0:t, sigma in SetSigma) + v_47b[t, s] * Gamma >= 0)
    @constraint(mrob, [t=T, s=ST, tau=t0:t, sigma=SetSigma], u_47b[t, s, tau, sigma] + v_47b[t, s] <= -f_47b[t, s, tau, sigma]*v_hat[tau, sigma])
    @constraint(mrob, [t=T, s=ST, tau=t0:t, sigma=SetSigma], u_47b[t, s, tau, sigma] + v_47b[t, s] <= f_47b[t, s, tau, sigma]*v_hat[tau, sigma])

    # (48): e[t](.) >= 0, forall t in T
    ...
    # ============== Dual budget constraints for eqn (48) =========== Min Case (>=)
    @variable(mrob, u_48[t=T, tau=t0:t, SetSigma] <= 0)
    @variable(mrob, v_48[t=T] <= 0)
    @constraint(mrob, [t=T], f0_48[t] + sum(v_bar[tau, sigma] * f_48[t, tau, sigma]
                + u_48[t, tau, sigma] for tau in t0:t, sigma in SetSigma) + v_48[t] * Gamma >= 0)
    @constraint(mrob, [t=T, tau=t0:t, sigma=SetSigma], u_48[t, tau, sigma] + v_48[t] <= -f_48[t, tau, sigma]*v_hat[tau, sigma])
    @constraint(mrob, [t=T, tau=t0:t, sigma=SetSigma], u_48[t, tau, sigma] + v_48[t] <= f_48[t, tau, sigma]*v_hat[tau, sigma])
    ...
    # (49a): x[t,s](.) <= 1, forall t in T, s in D
    ...
    # ============== Dual budget constraints for eqn (49a) =========== Max Case (<=)
    @variable(mrob, u_49a[t=T, D, tau=t0:t, SetSigma] >= 0)
    @variable(mrob, v_49a[t=T, D] >= 0)
    @constraint(mrob, [t=T, s=D], f0_49a[t, s] + sum((v_bar[tau,sigma] * f_49a[t, s, tau, sigma]
                + u_49a[t, s, tau, sigma]) for tau in t0:t, sigma in SetSigma) + v_49a[t, s] * Gamma <= 0)
    @constraint(mrob, [t=T, s=D, tau=t0:t, sigma=SetSigma], u_49a[t, s, tau, sigma] + v_49a[t, s] >= -f_49a[t, s, tau, sigma]*v_hat[tau, sigma])
    @constraint(mrob, [t=T, s=D, tau=t0:t, sigma=SetSigma], u_49a[t, s, tau, sigma] + v_49a[t, s] >= f_49a[t, s, tau, sigma]*v_hat[tau, sigma])

    # (49b): x[t,s](.) >= 0, forall t in T, s in D
    ...
    # ============== Dual budget constraints for eqn (49b) =========== Min Case (>=)
    @variable(mrob, u_49b[t=T, D, tau=t0:t, SetSigma] <= 0)
    @variable(mrob, v_49b[t=T, D] <= 0)
    @constraint(mrob, [t=T, s=D], f0_49b[t, s] + sum((v_bar[tau,sigma] * f_49b[t, s, tau, sigma]
                + u_49b[t, s, tau, sigma]) for tau in t0:t, sigma in SetSigma) + v_49b[t, s] * Gamma >= 0)
    @constraint(mrob, [t=T, s=D, tau=t0:t, sigma=SetSigma], u_49b[t, s, tau, sigma] + v_49b[t, s] <= -f_49b[t, s, tau, sigma]*v_hat[tau, sigma])
    @constraint(mrob, [t=T, s=D, tau=t0:t, sigma=SetSigma], u_49b[t, s, tau, sigma] + v_49b[t, s] <= f_49b[t, s, tau, sigma]*v_hat[tau, sigma])

    @objective(mrob, Min, E)
    println("===================================================================\nSolving robust budget model for t0 = $(t0)...")
    ...
    if JuMP.termination_status(mrob) == MOI.OPTIMAL
        ...
        println("\n===========  R O B U S T    B U D G E T    S O L U T I O N  ( t0 = $(t0) ) ===========\n")
        ...
        if t0 > 1  # if starting from period > 1
            for t in 1:(t0-1), c in 1:num_contracts[t]
                if size(fixed_contracts, 1) > 0
                    ...
                    sol_y[t,c] = -1
                    println("[Robust Budget Model, t0 = $(t0)] WARN: Returning invalid contract solution y. Check RCCP Robust Model procedure !")
                    println(general_logger, "[Robust Budget Model, t0 = $(t0)] WARN: Returning invalid contract solution y. Check RCCP Robust Model procedure !")
                end
            end
        end
        for t in T, c in 1:num_contracts[t]
    ...
    elseif has_values(mrob) && (JuMP.termination_status(mrob) in [MOI.TIME_LIMIT, MOI.ALMOST_OPTIMAL, MOI.ALMOST_LOCALLY_SOLVED, MOI.LOCALLY_SOLVED])
        ...
        rel_gap = MOI.get(mrob, MOI.RelativeGap())
        println("\n===========  R O B U S T    B U D G E T    S O L U T I O N  ( t0 = $(t0) ) ===========\n")
        println("[WARN] Time limit exceeded!")
        println("SubOptimal Objective Function value : ", objective_value(mrob))
        println("Relative gap = $(rel_gap)")
        println("Solve time : ", solve_time(mrob))
        println(general_logger, "[Robust Budget Model, t0 = $(t0)] WARN: Time limit exceeded. Optimal solution not found! Best bound: ", objective_bound(mrob))
        var_y = value.(y)
        if t0 > 1  # if starting from period > 1
            for t in 1:(t0-1), c in 1:num_contracts[t]
                if size(fixed_contracts, 1) > 0
        ...
        solution["x0"] = capture_variable_solution_as_array_2d(x0, t0, nbT, D)
        solution["x_"] = capture_variable_solution_as_array_4d(x_, t0, nbT, D, SetSigma)
        return objective_value(mrob), solution, solve_time(mrob), false, rel_gap
    else  # Infeasible
        println(infeasible_logger, "t0 = $(t0); Infeasible robust budget model : $(filepath)")
        println("[Robust Budget Model, t0 = $(t0)] Optimal solution not found! Best bound: ", objective_bound(mrob))
        println(general_logger, "[Robust Budget Model, t0 = $(t0)] WARN: Optimal solution not found! Best bound: ", objective_bound(mrob))
        solution["y"] = sol_y
